...
SELECT * FROM movies LEFT JOIN users on movie_id = user_id;
...
SELECT* FROM movies right JOIN users on user_id = movie_id;
...
SELECT count(*) FROM movies inner join users on favorite_movie_id = movie_id;
-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 4 >>>>>>>>>>>>>>>>>>>>>>>
-- Determine which lead studio's movies are favorited by users the most.
-- EXPECTED RESULT: Disney
SELECT movies.lead_studio, COUNT(*) AS count_of_occurrences FROM movies
JOIN users ON movies.movie_id = users.favorite_movie_id GROUP BY movies.lead_studio ORDER BY count_of_occurrences desc limit 1;
...
-- Get the average Rotten Tomatoes score of all movies that are favorited by a user.
SELECT AVG(movies.Rotten_Tomatoes) AS rotten_tomatoes
FROM movies
JOIN users ON movies.movie_id = users.favorite_movie_id;
...
select film, count(*) as duplicate_count from movies group by movies.film having Count(*) >1;
